...
// Update the weights values of the convolution values
inline __global__ void UpdateWeightsConvo(
	...
	fDevWeights[Idx] -= fWeightDecay*fLearnRate*fDevWeights[Idx] + fLearnMomentum*fLearnRate*sum;
}
...
void cFilterGroup::Train(int nCount){
	...
	if(Type == cGlobalValues::CONVOLUTION){
		...
		blockCount = ceil((double) Filters.size() * kerSize.width * kerSize.height * 1.0 / threadsPerBlock);

		UpdateWeightsConvo<<<blockCount, threadsPerBlock, 0, *CalcStream>>>( 
			DevInput[0]->data(), fDevDelta, fDevTemp, GlobalValues.fLearnStep,
			GlobalValues.fLearnMomentum, GlobalValues.fWeightDecay, kerSize, oSize, Filters.size());
